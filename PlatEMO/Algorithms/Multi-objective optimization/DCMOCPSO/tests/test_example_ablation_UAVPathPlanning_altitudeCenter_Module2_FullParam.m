function tests = test_example_ablation_UAVPathPlanning_altitudeCenter_Module2_FullParam
% Static checks for the module-2 altitude-center ablation.
tests = functiontests(localfunctions);
end

function testUsesModule2AltitudeConfigs(testCase)
scriptText = readScript();

verifyNotEmpty(testCase, regexp(scriptText, ...
    'altitudeCenterList\s*=\s*\[40,\s*50,\s*60,\s*70,\s*80,\s*90,\s*100\]\s*;', 'once'));
verifyNotEmpty(testCase, regexp(scriptText, ...
...
verifyNotEmpty(testCase, regexp(scriptText, ...
    'cacheDir\s*=\s*fullfile\(fileparts\(mfilename\(''fullpath''\)\),\s*''results'',\s*''altitude_center_module2_full_param''\)', 'once'));
verifyNotEmpty(testCase, regexp(scriptText, ...
    'AltitudeCenter_Module2_Full_H%d_B%d_%d_HV_objMetrics_runs\.mat', 'once'));
...
scriptPath = fullfile(scriptDir, 'example_ablation_UAVPathPlanning_altitudeCenter_Module2_FullParam.m');
