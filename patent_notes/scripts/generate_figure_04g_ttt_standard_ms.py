...
SUMMARY_FILE = (
    ...
    / "DCMOCPSO"
    / "results"
    / "ttt_standard_ms_full_param"
    / "TTT_StandardMs_Full_summary.mat"
)
OUTPUT_FILE = (
    ROOT
    / "patent_notes"
    / "figures"
    / "final_mixed"
    / "ttt_standard_ms_dcmocpso_metrics_lines.png"
)
...
def load_summary():
    ...
    results = mat["results"]
    labels = [str(int(value)) for value in np.asarray(mat["TTTListMs"], dtype=float)]
    data = {
...
def main():
    ...
    fig.suptitle("实验四：标准 TTT 敏感性", fontsize=18, fontweight="bold", y=0.98)
