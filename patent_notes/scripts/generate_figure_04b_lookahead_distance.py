...
SUMMARY_FILE = (
    ...
    / "DCMOCPSO"
    / "results"
    / "lookahead_distance_oneseg"
    / "LookaheadDistance_OneSeg_sweep_summary.mat"
)
OUTPUT_FILE = (
    ROOT
    / "patent_notes"
    / "figures"
    / "final_mixed"
    / "lookahead_distance_mocpso_metrics_lines.png"
)
...
def load_summary():
    ...
    distances = np.asarray(results.lookaheadDistanceList, dtype=float)
    mask = distances <= 1500
    data = {"x": distances[mask].astype(int)}
    for metric, *_ in METRICS:
        data[metric] = np.asarray(getattr(results, metric), dtype=float)[mask]
    return data
...
def main():
    ...
    fig.suptitle("实验四：前瞻距离敏感性", fontsize=18, fontweight="bold", y=0.98)
