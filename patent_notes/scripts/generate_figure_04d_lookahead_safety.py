...
SUMMARY_FILE = (
    ...
    / "LookaheadSafetyOneSegSweep_summary.mat"
)
OUTPUT_FILE = (
    ROOT
    / "patent_notes"
    / "figures"
    / "final_mixed"
    / "lookahead_safety_mocpso_metrics_lines.png"
)
...
def load_summary():
    ...
    data["fixed_hysteresis"] = float(results.fixedValue)
    return data
...
def main():
    ...
    fig.suptitle("实验四：安全裕量敏感性", fontsize=18, fontweight="bold", y=0.98)
    ...
    note_ax.text(
        0.04,
        0.62,
        f"固定动态滞后范围：{data['fixed_hysteresis']:.0f} dB\n折线点表示各参数取值下独立实验的均值。",
        ha="left",
        va="center",
        color="#666666",
        fontsize=11,
