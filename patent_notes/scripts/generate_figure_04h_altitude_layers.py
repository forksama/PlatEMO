...
SUMMARY_FILE = (
    ...
    / "DCMOCPSO"
    / "results"
    / "altitude_layers_module2_full_param"
    / "AltitudeLayers_Module2_Full_summary.mat"
)
OUTPUT_FILE = (
    ROOT
    / "patent_notes"
    / "figures"
    / "final_mixed"
    / "altitude_layers_dcmocpso_metrics_lines.png"
)
...
def load_summary():
    ...
    results = mat["results"]
    centers = np.asarray(mat["altitudeCenterList"], dtype=int)
    bounds = np.asarray(mat["altitudeBoundsList"], dtype=int)
    labels = [f"{center}\n({low}-{high})" for center, (low, high) in zip(centers, bounds)]
    data = {
...
def main():
    ...
    fig.suptitle("实验四：飞行高度区间敏感性", fontsize=18, fontweight="bold", y=0.98)

    for ax, (metric, ylabel, subtitle, direction, color) in zip(axes_flat, METRICS):
        draw_line(ax, data["x"], data["labels"], data[metric], ylabel, subtitle, direction, color)
